        ...
        var Utils = (function () {
            ...
            */
            Utils.pointInPolygon = function (x, y, corners, startX, startY) {
                // Pick a ray origin guaranteed to sit outside the polygon.
...
(function (BP3D) {
    ...
    (function (Items) {
        ...
        var FloorItem = (function (_super) {
            ...
            /** Center point must be inside a room; footprint overlap with walls is allowed. */
                ...
                var loaderCallback = function (geometry, materials) {
                    try {
                        if (!geometry || !geometry.vertices || geometry.vertices.length === 0) {
                            console.error('Blueprint3D: empty geometry for', fileName);
                            return;
                        }
                        if (!materials || materials.length === 0) {
                            materials = [new THREE.MeshLambertMaterial({ color: 0xcccccc })];
                        }
                        // Legacy models are sometimes authored in metres; normalise to cm.
                        geometry.computeBoundingBox();
                        var bbox = geometry.boundingBox;
                        var spanX = bbox.max.x - bbox.min.x;
                        var spanZ = bbox.max.z - bbox.min.z;
                        var maxHoriz = Math.max(spanX, spanZ);
                        if (maxHoriz > 0 && maxHoriz < 5) {
                            geometry.scale(100, 100, 100);
                            geometry.computeBoundingBox();
                        } else if (maxHoriz >= 5 && maxHoriz < 20) {
                            geometry.scale(10, 10, 10);
                            geometry.computeBoundingBox();
                        }
                        var ItemClass = BP3D.Items.Factory.getClass(itemType);
                        if (!ItemClass) {
                            console.error('Blueprint3D: unknown item type', itemType, 'for', fileName);
                            return;
                        }
                        var item = new ItemClass(scope.model, metadata, geometry, new THREE.MeshFaceMaterial(materials), position, rotation, scale);
                        ...
                        console.error('Blueprint3D: failed to create item from', fileName, err);
                    }
                };
...
(function (BP3D) {
    ...
    (function (Floorplanner) {
        ...
        // grid parameters — light plan-sheet theme (Neo Foyr style)
        var planMargin = 48;
        var gridSpacing = 20;
        var gridMajorEvery = 5;
        var gridMinorColor = "rgba(60, 50, 40, 0.06)";
        var gridMajorColor = "rgba(60, 50, 40, 0.14)";
        var canvasBg = "#f5f1ea";
        var marginBg = "#ebe6dc";
        var roomColor = "#ffffff";
        var roomLabelColor = "#374151";
        var roomAreaColor = "#6b7280";
        // wall config
        var wallWidth = 6;
        var wallWidthHover = 7;
        var wallColor = "#3d3836";
        var wallColorHover = "#6366f1";
        var edgeColor = "#3d3836";
        var edgeColorHover = "#6366f1";
        var edgeWidth = 1;
        var deleteColor = "#ef4444";
        var dimLineColor = "#6b7280";
        var dimTextColor = "#111827";
        // corner config
        var cornerRadius = 0;
        var cornerRadiusHover = 6;
        var cornerColor = "#78716c";
        var cornerColorHover = "#6366f1";
        /**
        ...
        var FloorplannerView = (function () {
            ...
            FloorplannerView.prototype.draw = function () {
                ...
                var m = planMargin;
                this.context.fillStyle = marginBg;
                this.context.fillRect(0, 0, w, h);
                this.context.fillStyle = canvasBg;
                this.context.fillRect(m, m, Math.max(0, w - m * 2), Math.max(0, h - m * 2));
                this.context.save();
                this.context.beginPath();
                this.context.rect(m, m, Math.max(0, w - m * 2), Math.max(0, h - m * 2));
                this.context.clip();
                this.drawGrid();
                ...
                this.context.strokeStyle = "rgba(60, 50, 40, 0.18)";
                this.context.lineWidth = 1;
                this.context.strokeRect(m + 0.5, m + 0.5, Math.max(0, w - m * 2 - 1), Math.max(0, h - m * 2 - 1));
            };
            /** */
            ...
            FloorplannerView.prototype.drawWall = function (wall) {
                ...
                if (wall.frontEdge) {
                    this.drawWallFill(wall.frontEdge, color);
                }
                if (wall.backEdge) {
                    this.drawWallFill(wall.backEdge, color);
                }
            };
            FloorplannerView.prototype.drawWallFill = function (edge, color) {
            ...
            FloorplannerView.prototype.drawEdgeLabel = function (edge) {
                ...
                var c1 = edge.interiorStart();
                var c2 = edge.interiorEnd();
                var x1 = this.viewmodel.convertX(c1.x);
                var y1 = this.viewmodel.convertY(c1.y);
                var x2 = this.viewmodel.convertX(c2.x);
                var y2 = this.viewmodel.convertY(c2.y);
                var dx = x2 - x1;
                var dy = y2 - y1;
                ...
                var ny = dx / segLen;
                var offset = 26;
                var lx1 = x1 + nx * offset;
                var ly1 = y1 + ny * offset;
                var lx2 = x2 + nx * offset;
                var ly2 = y2 + ny * offset;
                var mx = (lx1 + lx2) / 2;
                var my = (ly1 + ly2) / 2;
                var tick = 6;
                this.context.strokeStyle = dimLineColor;
                this.context.lineWidth = 1;
                this.context.setLineDash([3, 3]);
                this.context.beginPath();
                this.context.moveTo(x1 + nx * 8, y1 + ny * 8);
                this.context.lineTo(lx1, ly1);
                this.context.moveTo(x2 + nx * 8, y2 + ny * 8);
                ...
                this.context.stroke();
                this.context.beginPath();
                this.context.moveTo(lx1 - nx * tick, ly1 - ny * tick);
                this.context.lineTo(lx1 + nx * tick, ly1 + ny * tick);
                this.context.moveTo(lx2 - nx * tick, ly2 - ny * tick);
                this.context.lineTo(lx2 + nx * tick, ly2 + ny * tick);
                this.context.stroke();
                this.context.font = "500 11px -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
                this.context.fillStyle = dimTextColor;
                this.context.textBaseline = "middle";
                this.context.textAlign = "center";
                this.context.fillText(BP3D.Core.Dimensioning.cmToMeasure(length), mx, my);
            };
            /** */
            ...
            FloorplannerView.prototype.drawRoom = function (room) {
                ...
                this.drawPolygon(BP3D.Core.Utils.map(room.corners, function (corner) {
                    ...
                    return scope.viewmodel.convertY(corner.y);
                }), true, roomColor, true, "rgba(60, 50, 40, 0.08)", 1);
                if (!room.corners || room.corners.length < 3) {
                    return;
                }
                var cx = 0;
                var cy = 0;
                ...
                for (var i = 0; i < room.corners.length; i++) {
                    cx += room.corners[i].x;
                    cy += room.corners[i].y;
                    var j = (i + 1) % room.corners.length;
                    area += room.corners[i].x * room.corners[j].y;
                    area -= room.corners[j].x * room.corners[i].y;
                }
                cx /= room.corners.length;
                cy /= room.corners.length;
                area = Math.abs(area / 2);
                var areaFt2 = area / 929.0304;
                var px = this.viewmodel.convertX(cx);
                var py = this.viewmodel.convertY(cy);
                this.context.font = "600 13px -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
                this.context.fillStyle = roomLabelColor;
                this.context.textAlign = "center";
                this.context.textBaseline = "middle";
                this.context.fillText("Room", px, py - 8);
                this.context.font = "500 12px -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
                this.context.fillStyle = roomAreaColor;
                this.context.fillText(areaFt2.toFixed(1) + " ft\u00B2", px, py + 10);
            };
            /** */
            ...
            FloorplannerView.prototype.drawGrid = function () {
                ...
                for (var x = 0; x <= (width / gridSpacing); x++) {
                    var isMajor = x % gridMajorEvery === 0;
                    this.drawLine(gridSpacing * x + offsetX, 0, gridSpacing * x + offsetX, height, 1, isMajor ? gridMajorColor : gridMinorColor);
                }
                for (var y = 0; y <= (height / gridSpacing); y++) {
                    var isMajorY = y % gridMajorEvery === 0;
                    this.drawLine(0, gridSpacing * y + offsetY, width, gridSpacing * y + offsetY, 1, isMajorY ? gridMajorColor : gridMinorColor);
                }
            };
    ...
    (function (Floorplanner_1) {
        ...
        var Floorplanner = (function () {
            ...
            Floorplanner.prototype.resetOrigin = function () {
                var margin = 48;
                var centerX = margin + (this.canvasElement.innerWidth() - margin * 2) / 2.0;
                var centerY = margin + (this.canvasElement.innerHeight() - margin * 2) / 2.0;
    ...
    (function (Three) {
        Three.Controller = function (three, model, camera, element, controls, hud) {
            ...
            function normalizeVector2(vec2) {
                ...
                var w = three.elementWidth || (window.innerWidth - three.widthMargin);
                var h = three.elementHeight || (window.innerHeight - three.heightMargin);
                retVec.x = ((vec2.x - three.widthMargin) / w) * 2 - 1;
                retVec.y = -((vec2.y - three.heightMargin) / h) * 2 + 1;
                return retVec;
            }
            //
